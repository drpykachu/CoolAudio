...
def square(length):
    ...
    x1 = list(np.linspace(0,length/2,int(chunk_size/4)))
    x2 = list(np.zeros(int(chunk_size/4))+length/2)
    x3 = list(np.linspace(length/2,0,int(chunk_size/4)))
    x4 = list(np.zeros(int(chunk_size/4)))
    
    y1 = list(np.zeros(int(chunk_size/4)))
    y2 = list(np.linspace(0,length/2,int(chunk_size/4)))
    y3 = list(np.zeros(int(chunk_size/4))+length/2)
    y4 = list(np.linspace(length/2,0,int(chunk_size/4)))

    return [np.array(x1+x2+x3+x4)+WIDTH/2-length/4,np.array(y1+y2+y3+y4)+HEIGHT/2-length/4]
...
while running:
    ...
    for i in range(len(left_channel_normalized) - 1):
        ...
        pygame.draw.line(screen, (0, 255, 255), (int(x1), int(y1)), (int(x2), int(y2)), 5)  # Draw a line between consecutive points
